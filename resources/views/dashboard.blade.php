...
@section('content')
<div
  class="dashboard-container grid grid-cols-12 gap-4 p-6"
  style="font-family: Lexend, 'Noto Sans', sans-serif;"
>
    <!-- Good Afternoon Section -->
    <div class="col-span-12 md:col-span-8 bg-gradient-to-r from-blue-400 to-blue-500 text-white rounded-lg p-6 shadow-md">
        <h1 class="text-2xl font-bold">
            Good 
            @if(now()->hour < 12) Morning 
            @elseif(now()->hour < 18) Afternoon 
            @else Evening 
            @endif, {{ $userName }}.
        </h1>
    ...
    <div class="col-span-12 md:col-span-4 bg-white rounded-lg p-4 shadow-md"  >
        ...
        <p class="text-sm mt-2">No items found</p>
    </div>
        ...
        @forelse ($tasks as $task)
            <li class="flex justify-between items-center bg-gray-50 p-2 rounded-lg mb-2">
                <div>
                    ...
                    <p class="text-sm text-gray-400">{{ $task->deadline ? $task->deadline->format('d M Y, H:i') : 'No deadline' }}</p>
                </div>
                <div class="flex gap-2">
                    <!-- Edit Button -->
<button type="button" onclick="window.location.href='{{ route('tasks.edit', $task->id) }}'" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900">
    Edit
</button>
...
<button type="button" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" data-modal-target="deleteTaskModal" data-task-id="{{ $task->id }}" data-modal-toggle="deleteTaskModal">
   ...
   <!-- Focus Timer -->
<div class="col-span-12 md:col-span-4 bg-gradient-to-r from-blue-400 to-blue-500 text-white rounded-lg p-4 shadow-md">
    <h3 class="text-lg font-bold mb-3">Focus Timer</h3>
    <p id="timer-display" class="text-4xl font-bold">25:00</p>
    <div class="mt-3 flex gap-2">
        <button id="start-timer" class="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100">Start</button>
        <button id="pause-timer" class="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100 hidden">Pause</button>
        <button id="reset-timer" class="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100 hidden">Reset</button>
    </div>
</div>
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <div class="p-6 text-center">
                ...
                <form id="deleteTaskForm" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-white bg-red-600 hover:bg-red-800 px-5 py-2.5 rounded-lg">Yes, delete it</button>
                    <button type="button" data-modal-hide="deleteTaskModal" class="text-gray-500 bg-white hover:bg-gray-100 px-5 py-2.5 rounded-lg">No, cancel</button>
                </form>
            </div>
        </div>
    </div>
