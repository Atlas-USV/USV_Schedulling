<form class="p-4 md:p-5">
    <div class="grid gap-4 mb-4 grid-cols-2">
        ...
        <!-- Examinatori -->
        <div class="col-span-2 sm:col-span-1">
            <label for="retake_examinators" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Examinatori</label>
            <select id="retake_examinators" multiple="" data-hs-select='{
               "hasSearch": true,
...
<script>
    ...
    $(document).ready(function() {
        // const selectGroup = HSSelect.getInstance(`#retake_group_id`);
        const selectTeacher = HSSelect.getInstance(`#retake_teacher_id`);
        const selectRoom = HSSelect.getInstance(`#retake_room_id`);
        const selectSubject = HSSelect.getInstance(`#retake_subject_id`);
        const examinators = window.HSSelect.getInstance('#retake_examinators', true);

        function initialPopulation() {
            window.populateDropdown('retake_teacher_id', cadri, cadriPlaceholder, 'id', {
                'name': ''
            }, selectTeacher);
    ...
    $('#submitRetake').click(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#retake_start_time').val('');
                $('#retake-number-input').val('');
                ...
                $('#retake-durata-error-message').addClass('hidden');
            },
